			...
			function creaAttivita(result){
				...
				for(var i=0; i<result.length;i++){
					...
					if(result[i].Pubblica=="si"){
						...
						html.find(".thumb_attivita").pwi({
				            username: commonsSettings.picasaUser,
				            mode: 'albums',
							albumThumbSize: 190,
							maxResults: 10,
				            albums: ["Att"+nomeAlbumPicasa],
				            showAlbumTitles: false,
				            onclickAlbumThumb: function (e) {
				                return false;//alert('We got the event object and the albumstream returned.\n\n');
				            }
				        });
			...
			function singolaAttivita(nome,categoria,citta,indirizzo,descrizione,email,tell,cell,sito,tags,lat,lon,fb,tw,yt){
				...
				var html='<div class="container clearfix box_attivita" id="box_attivita_'+id+'" data-nome="'+nome+'"  data-categoria="'+categoria+'" data-citta="'+citta+'" data-tags="'+tags+'" >'+
							'<div class="label_box coloreAttivita">Attività</div>'+
			              	'<div  class="grid_4" style="width:525px; float:left; margin:0px 0px -10px 0px">'+
			              ...
			              html+='</div>'+
			              		...
			              		'<h2 style="margin:0px 0px 6px 0px">'+nome+'</h2>'+						
								'<ul class="latestpost">'+
									'<li>'+
										'<h4>Categoria: <a  rel="bookmark" >'+categoria+'</a></h4>'+
										'<h4>Città: <a  rel="bookmark" class="citta" >'+citta+'</a></h4>'+
										'<h4>Indirizzo: <a  rel="bookmark" >'+indirizzo+'</a></h4>'+descrizione+''+
			              				
			              				'<div style="margin:8px 0px 0px 0px">'+
			              					'<a title="" rel="" class="button button_mappa_attivita">Come raggiungerci</a>';
			              					if(email!="non presente") html+='<a data-href="mailto:'+email+'" title="" rel="" class="button">E-mail</a>';
			              					if(tell!="non presente") html+='<a data-href="callto&#58;+'+tell+'" title="" rel="" class="button">Telefono</a>';
			              					if(cell!="non presente") html+='<a data-href="callto&#58;+'+cell+'" title="" rel="" class="button">Cellulare</a>';
			              					if(sito!="non presente") html+='<a data-href="http://'+sito+'" target="_blanck" title="" rel="" class="button">Sito</a>';
							 ...
							 '</div>'+
							 '<div style="float:left;"><div style="position:relative"><div style="position:absolute" class="thumb_attivita"><div style="position:absolute;z-index:5"><img src="source/css/images/loading_photos.gif"></div></div>'+
			...
			function pageNavigatorAttivitaCategoria(arr){
				$.each(arr,function(index,value){
					var html_tasto='<li id="menu-item-"'+index+'><a>'+value+'</a></li>';
					var html_tasto_home='<a class="button_home">'+value+'</a>';
					
					var tasto=$(html_tasto).click(function(){
												var cat=$(this).find("a").html();
												$(".box_attivita").hide();
												$(".box_attivita").each(function(){
													if($(this).attr("data-categoria")==cat)
														$(this).show();
												});
												setTimeout(function(){
													updateMapAttivita("3")
			...
			function pushaPuntoSuMappaAttivita(nome,categoria,indirizzo,citta,tell,cell,sito,lat,lon){
				...
				var html='<div id="punto_attivita_'+id+'" class="map-location" data-jmapping="{id: '+Math.floor(Math.random()*101)+', point: {lng: '+lon+', lat: '+lat+', category: '+categoria+'}}" >'+
		...
		function mostraDettaglioAttivita(el){
			...
			if(!$(".overlay").is(":visible")){
				...
				.find(".thumb_attivita").parent().html("").pwi({
					 username: commonsSettings.picasaUser,
			...
			function eventi(result){
				...
				for(var i=0; i<result.length;i++){
					if(result[i].Pubblica=="si"){
						...
						arrCategorie.push(result[i].Categoria);
						arrCitta.push(citta);
						var nomeAlbumPicasa=picasaAlbumName(result[i].Nome_evento)
						html.find(".thumb_evento").pwi({
				            username: commonsSettings.picasaUser,
				            mode: 'albums',
							albumThumbSize: 190,
							maxResults: 10,
				            albums: ["Eve"+nomeAlbumPicasa],
				            showAlbumTitles: false,
				            onclickAlbumThumb: function (e) {
				                alert('We got the event object and the albumstream returned.\n\n');
				            }
				        });
			...
			function pageNavigatorEventoCategoria(arr){
				$.each(arr,function(index,value){
					var html_tasto='<li id="menu-item-evento"'+index+'><a>'+value+'</a></li>';
			              	...
			              	'<div class="grid_4" style="width:700px; float:left; margin:0px 0px 20px 0px">'+                
								...
								'<ul class="latestpost">'+
									'<li>'+
										...
										'<h4>città: <a>'+citta+'</a></h4>'+descrizione+''+
			              				
			              				'<div style="margin:8px 0px 0px 0px" class="buttons_evento">'+
			              					'<a title="" rel="" class="button button_mappa_evento">Come raggiungerci</a>';
			              					if(email!="non presente") html+='<a data-href="mailto:'+email+'" title="" rel="" class="button">E-mail PR</a>';
			              					if(tell!="non presente") html+='<a data-href="callto&#58;+'+tell+'" title="" rel="" class="button">Telefono PR</a>';
			              					if(cell!="non presente") html+='<a data-href="callto&#58;+'+cell+'" title="" rel="" class="button">Cellulare PR</a>';
								...
								'</ul>'+
							 '</div>'+
							 '<div style="float:left;"><div style="position:relative"><div style="position:absolute;background-image:url(source/css/images/no_photo.png);width: 210px;height: 210px;" class="thumb_evento"><div style="position:absolute;z-index:5"><img src="source/css/images/loading_photos.gif"></div></div>'+
			...
			function pushaPuntoSuMappaEventi(nome,nome_attivita,categoria,citta,tell,cell,lat,lon){
				...
				var categoria="'"+categoria+"'";
				id=picasaAlbumName(nome);
				var html='<div id="punto_evento_'+id+'" class="map-location" data-jmapping="{id: '+Math.floor(Math.random()*101)+', point: {lng: '+lon+', lat: '+lat+', category: '+categoria+'}}" >'+
						    '<a href="#" class="map-link">'+nome+'</a>'+
						    '<div class="info-box">'+
						      	'<p><h2 style="margin:0px 0px 6px 0px; font-size:20px">'+nome+'</h2></p>'+
						    ...
						    '</div>'+
						  '</div>';
  				$('#hidden_map-side-bar_eventi').append(html);
			}
		...
		function pompaListaEventiAttivita(id_attivita,id_evento,nome,data){
			...
			var tasto=$("<div><div style='padding:5px 0px 0px 5px; margin:0px 0px -3px -3px'>"+data+"</div><div style='font-family: Lobster; font-size:18px'><a class='coloreEvento'>"+nome+"</a></div></div>");
