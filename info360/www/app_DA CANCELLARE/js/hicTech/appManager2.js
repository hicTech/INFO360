 ...
 * http:// appml.org
 *
 * Copyright 2011, hicTech srl www.hictech.comback
...
(function($) {
    ...
    $.jQTouch = function(options) {
		...
		function setCurrentPage(_currentPage){
		    ...
		    refreshSelectedPanel();
		    appML.notifyPageChange(currentPage.attr("id"),selectedPanel.getId());
		}
...
(function($){
	...
	$.iScroll.prototype = {
		...
		setCurrentViewport: function(x,y,p_w,p_h,l_w,l_h){
			currentViewport= {
				x:x,
				y:y,
				p_w:p_w,
		...
		setPosition: function (x, y, hideScrollBars) {
			...
			var p_w=that.wrapper_dims.portrait.width;
			var p_h=that.wrapper_dims.portrait.height;
			var l_w=that.wrapper_dims.landscape.width
			var l_h=that.wrapper_dims.landscape.height
			
			
			that.setCurrentViewport(x,y,p_w,p_h,l_w,l_h);
		...
		scrollToElement: function (el, runtime) {
			...
			var $el=$(el);
			var vertical_el_dims=	parseInt($el.css("height"))+parseInt($el.css("padding-top"))+parseInt($el.css("padding-bottom"))+
									parseInt($el.css("margin-bottom"))+parseInt($el.css("margin-top"))+parseInt($el.css("border-bottom-width"))
									+parseInt($el.css("border-top-width"));
			
			y=parseInt(y+vertical_el_dims);
			that.scrollTo(x, y, runtime);
		},
		
		scrollTop: function (el, runtime) {
			el = typeof el == 'object' ? el : this.element.querySelector(el);
...
/*$(document).ready(*/(function($){
	...
	$.appManager = function(options){
		...
		*/
		...
		function injectTitlePage(title,login,i,toolButton){
			if(toolButton==undefined)
				toolButton="";
			var toolBvisibility="none";
			...
			if(toolButton=="eve")
					toolButton="<a class='toolButton flip'  id='mappa_button_eve' href='#page_mappa_eve'>Mappa eve</a>"
			if(toolButton=="att")
					toolButton="<a class='toolButton flip'  id='mappa_button_att' href='#page_mappa_att'>Mappa att</a>"
			
				$("#appML_navigation_bar").append($("<div style='position:relative'><div style='"+title_visibility+"'>"+title+" "+toolButton+"</div><div class='back'>Back</div></div>"));
			}
		}
		...
		function animate(fromPage, toPage, animation, backwards){
			...
			fromPageJQT=fromPage
		}
		
		function proveniencePage(){
			return fromPageJQT;
		}
		...
		function getScrollElemDimensions($elem){
			if($elem.attr("class")=="iscroll" && appML.getEnviroment().isIDevice){
				
				var h_m=parseInt($elem.css("height"));
				if( appML.getEnviroment().isIPad)
					return { landscape: { width:1024, height:h_m }, portrait:{ width:768, height:h_m } }
				else
					return { landscape: { width:480, height:h_m }, portrait:{ width:320, height:h_m } }
			}
        ...
        function showSearchBar(){
			...
			console.log("tempo for daiRigaAttivita: "+ parseInt(time6-time5))
			
			
			var time3=new Date(); time3=time3.getTime();
        	$('#overlay_search').addClass('overlay_search_on');
        	var time4=new Date(); time4=time4.getTime();
	    ...
	    function getPanelHtml($panel, standard_tags){
	    	...
	    	var toolB=getAppmlAttribute($panel,"toolButton");
			toolB = (toolB!=null) ? " data-toolButton='"+toolB+"'" : "";

	    	var html="<div "+wrapper_id+title+icon+toolB+"><div "+id+" class='appML_panel appML_div'>";
		...
		this.appManagerHideDialog=function(options){
			hideDialog();
		};
		...
		this.getEnviroment = function(){
			...
			isStandalone = hasHomescreen && nav.standalone, /// on homescreen
		...
		this.proveniencePage = function(){
			return proveniencePage();
		}
		...
		function notifyCarouselSnap(id, position, horizontal, other_position){
			if(appML.carousel_listeners!=null){
				...
				if(arr!=null){
					for(var i=0;i<arr.length;i++)
						arr[i].call(null,id,position,selected_elem,horizontal, other_position);
				}
		...
		*/
		...
		this.getCurrentViewport = function(){
			if(this.landscapeScreen()){
				return {
					t_l:[currentViewport.x*-1,currentViewport.y*-1],
					t_r:[parseInt(currentViewport.x-currentViewport.l_w)*-1,currentViewport.y*-1],
					b_l:[currentViewport.x*-1,parseInt(currentViewport.y-currentViewport.l_h)*-1],
					b_r:[parseInt(currentViewport.x-currentViewport.l_w)*-1,parseInt(currentViewport.y-currentViewport.l_h)*-1]
				}
			}
			else{
				return {
					t_l:[currentViewport.x*-1,currentViewport.y*-1],
					t_r:[parseInt(currentViewport.x-currentViewport.p_w)*-1,currentViewport.y*-1],
					b_l:[currentViewport.x*-1,parseInt(currentViewport.y-currentViewport.p_h)*-1],
					b_r:[parseInt(currentViewport.x-currentViewport.p_w)*-1,parseInt(currentViewport.y-currentViewport.p_h)*-1]
				}
			}
		}
		...
		this.notifyPageChange = function(page_id, panel_id){
			var $elem = $("#"+page_id+"_appML_container");
			if($elem.length>0){
				var w = $elem.attr("data-appml-w");
				var h = $elem.attr("data-appml-h");
				var callback = $elem.attr("data-appml-callback");
				appML.makeScrollable($elem,"console.log('Terminato un div di prova!');",w,h);
			}
			...
			if(appML.page_listeners!=null){
				...
				if(arr!=null){
					for(var i=0;i<arr.length;i++)
						arr[i].call(null,page_id,panel_id);
				}
		...
		this.scrollTop = function(elem){
			var $elem = $(elem);
			elem = $elem.get(0);
			var iscroll_id = $elem.parents(".iscroll").attr("id");
			var iscroll = getIScroll(iscroll_id);
			if(iscroll!=null)
				iscroll.scrollTop(elem);
			else
				console.log("iscroll not found for "+iscroll_id);
		};
	...
	var r_conf;	
	var r_paragrafi_home;
	var r_news_home;
	
	$(window).load(function(){
		startMultipleRequest(commonsSettings.requests_queue,parti);
	});
