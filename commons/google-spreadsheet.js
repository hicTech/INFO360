      ...
      googleSpreadsheet.load(function(result) {
		...
		for(var i=1;i<myJsonArray.length;i++){
			...
			for(var j=0;j<myJsonArray[i].length;j++){
				...
				// se in una cella viene messo un a capo finale questo viene codificato in %20A
				// quindi verifico se la stringa "valore" finisce per %20A lo elimino
				if(valore.indexOf("%20A")!=-1 && valore.indexOf("%20A")==valore.length-4){  
					valore = valore.substr(0,valore.length-4);
				}
